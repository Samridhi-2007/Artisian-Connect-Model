...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("[v0] Login successful, setting cookie and redirecting")
        document.cookie = `auth-token=${data.token}; path=/; max-age=86400`
        router.push(`/${redirectTo}`)
      } else {
        const errorData = await response.json()
        console.error("[v0] Login failed:", errorData.message)
  ...
  const handleAdminSubmit = async (e: React.FormEvent) => {
    ...
    console.log("[v0] Attempting admin login with:", { email, adminCode })
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        console.log("[v0] Admin login successful, setting cookie and redirecting")
        document.cookie = `auth-token=${data.token}; path=/; max-age=86400`
        router.push("/admin")
      } else {
        const errorData = await response.json()
        console.error("[v0] Admin login failed:", errorData.error)
        alert(errorData.error || "Admin login failed")
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-2">AI-Powered Marketplace for Handmade Crafts</p>
        </div>

        <Card className="shadow-lg">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl">Welcome Back</CardTitle>
            <CardDescription>Sign in to your account to continue your artisan journey</CardDescription>
          </CardHeader>
            ...
            <Tabs defaultValue="user" className="w-full">
              ...
              <TabsContent value="admin" className="space-y-4">
                <form onSubmit={handleAdminSubmit} className="space-y-4">
                  ...
                  <Button
                    type="submit"
                    className="w-full h-11 text-base bg-destructive hover:bg-destructive/90"
                    disabled={isLoading}
                  >
